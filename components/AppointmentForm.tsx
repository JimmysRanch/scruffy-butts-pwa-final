...
export default function AppointmentForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      // 1) write to bookings (returns id)
      ...
      // basic reset
      setForm({ ...form, ownerName: '', petName: '', date: '', time: '', notes: '' });
    } catch (e: any) {
  ...
  return (
    <form onSubmit={submit} style={{ maxWidth: 520, margin: '0 auto', padding: 16 }}>
      ...
      <label>Pet name
        <input name="petName" value={form.petName} onChange={change} required
               style={{ display: 'block', width: '100%', padding: 8, margin: '6px 0 12px' }} />
      </label>

      <label>Size
        <select name="size" value={form.size} onChange={change}
                style={{ display: 'block', width: '100%', padding: 8, margin: '6px 0 12px' }}>
